...
const BookPage = () => {
  ...
  const [filterTitle, setFilterTitle] = useState(''); // Tìm kiếm theo tiêu đề
  const [filterError, setFilterError] = useState(''); // Lỗi từ backend
  const pageSize = 5;
  ...
  // Fetch books - giống hệt FinePage
  const fetchBooks = useCallback(async () => {
    ...
    try {
      ...
      // Chọn endpoint: tìm kiếm hay danh sách
      const endpoint = filterTitle ? endpoints['find-book-by-title'] : endpoints.books;
      if (filterTitle) {
        params.title = filterTitle.trim();
      }
      ...
      if (err.response?.status === 404) {
        setFilterError('Không tìm thấy sách nào với tiêu đề này!');
      } else if (err.response?.status === 403) {
        setFilterError('Bạn không có quyền tìm kiếm sách!');
      } else {
        setFilterError(`Lỗi khi tìm kiếm: ${err.response?.data?.message || err.message}`);
      }
    } finally {
      setLoading(false);
    }
  }, [currentPage, pageSize, filterTitle]);
        ...
        <div className={`relative top-20 mx-auto p-8 border w-full max-w-4xl shadow-2xl rounded-3xl bg-white transition-transform duration-300 ${show ? 'scale-100' : 'scale-95'}`}>
          ...
          {selectedBook && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                {selectedBook.image ? (
                  <img src={selectedBook.image} alt={selectedBook.title} className="w-full md:w-64 h-auto md:h-80 object-cover rounded-xl shadow-lg mb-4" />
                ) : (
              ...
              <div className="space-y-4 text-gray-700">
                <p className="flex items-center"><span className="font-semibold w-32 text-gray-900">ID:</span> <span>{selectedBook.id}</span></p>
                <p className="flex items-center"><span className="font-semibold w-32 text-gray-900">Tiêu đề:</span> <span>{selectedBook.title}</span></p>
                <p className="flex items-center"><span className="font-semibold w-32 text-gray-900">Tác giả:</span> <span>{selectedBook.author}</span></p>
                <p><span className="font-semibold w-32 text-gray-900 block md:inline-block">Mô tả:</span> <span className="text-sm italic">{selectedBook.description || 'Không có mô tả'}</span></p>
                <p className="flex items-center"><span className="font-semibold w-32 text-gray-900">Năm xuất bản:</span> <span>{selectedBook.publishedDate}</span></p>
                <p className="flex items-center"><span className="font-semibold w-32 text-gray-900">Nhà xuất bản:</span> <span>{selectedBook.publisher}</span></p>
                <p className="flex items-center"><span className="font-semibold w-32 text-gray-900">Ngôn ngữ:</span> <span>{selectedBook.language}</span></p>
                <p className="flex items-center"><span className="font-semibold w-32 text-gray-900">ISBN-10:</span> <span>{selectedBook.isbn10}</span></p>
                <p className="flex items-center"><span className="font-semibold w-32 text-gray-900">ISBN-13:</span> <span>{selectedBook.isbn13}</span></p>
                <p className="flex items-center"><span className="font-semibold w-32 text-gray-900">Giá:</span> <span>{selectedBook.price.toLocaleString('vi-VN')} VND</span></p>
                <p className="flex items-center"><span className="font-semibold w-32 text-gray-900">Sách in:</span> <span>{selectedBook.isPrinted ? 'Có' : 'Không'}</span></p>
                {selectedBook.isPrinted && (
                  <>
                    <p className="flex items-center"><span className="font-semibold w-32 text-gray-900">Vị trí kệ:</span> <span>{selectedBook.shelfLocation || 'Không có'}</span></p>
                    <p className="flex items-center"><span className="font-semibold w-32 text-gray-900">Tổng số bản sao:</span> <span>{selectedBook.totalCopy || 0}</span></p>
                    <p className="flex items-center"><span className="font-semibold w-32 text-gray-900">Số sách đang mượn:</span> <span>{selectedBook.borrowCount || 0}</span></p>
                  </>
                )}
                <p className="flex items-center"><span className="font-semibold w-32 text-gray-900">Sách điện tử:</span> <span>{selectedBook.isElectronic ? 'Có' : 'Không'}</span></p>
                {selectedBook.isElectronic && (
                  <>
                    <p className="flex items-center"><span className="font-semibold w-32 text-gray-900">Định dạng:</span> <span>{selectedBook.format || 'Không có'}</span></p>
                    <p className="flex items-center"><span className="font-semibold w-32 text-gray-900">Giấy phép:</span> <span>{selectedBook.licence || 'Không có'}</span></p>
                    <p className="flex items-center"><span className="font-semibold w-32 text-gray-900">Số lượt xem online:</span> <span>{selectedBook.totalView || 0}</span></p>
                  </>
                )}
                <p className="flex items-center">
                  ...
                  <span className="flex flex-wrap gap-2">
                    {selectedBook.categories?.map(cat => (
                      <span key={cat.id} className="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded-full">{cat.name}</span>
                    ))}
                  </span>
                </p>
                <p className="flex items-center"><span className="font-semibold w-32 text-gray-900">Ngày tạo:</span> <span>{new Date(selectedBook.createdDate).toLocaleString('vi-VN')}</span></p>
                <p className="flex items-center"><span className="font-semibold w-32 text-gray-900">Ngày cập nhật:</span> <span>{new Date(selectedBook.updatedDate).toLocaleString('vi-VN')}</span></p>
              </div>
            </div>
          )}
        </div>
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      {/* === FILTER SECTION - GIỐNG HỆT FINEPAGE === */}
      <div className="mb-6">
        ...
        <div className="flex items-center gap-4">
          <input
            ...
            aria-label="Lọc theo tiêu đề sách"
          />
          <button
            onClick={clearFilter}
                  ...
                  books.map((book) => (
                    <tr key={book.id} className="hover:bg-blue-50 transition-colors duration-200">
                      <td className="py-4 px-6">
                        {book.image ? (
                          <img src={book.image} alt={book.title} className="w-14 h-20 object-cover rounded-lg shadow-md" />
                        ) : (
